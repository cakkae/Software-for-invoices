...
public class NovaFaktura {
	...
	private JTextField tfUcitajArtikle;
	...
	public static void main(String[] args) {
		try {
            ...
            java.util.logging.Logger.getLogger(NovaFaktura.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					NovaFaktura window = new NovaFaktura();
	...
	public NovaFaktura() throws ParseException {
		initialize();
	}
	...
	private void initialize() throws ParseException {
				...
				if (!tfBrojFakture.getText().isEmpty())
				{
					...
					timer.schedule(new TimerTask()
					{
						...
						public void run()
						{
							...
							rezultatPostojiBrojFakture = db.provjeriBrojFakture(tfBrojFakture.getText());
		...
		lSpremiFakturu.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) 
			{
				...
				else
				{
					...
					{
						DBHome db = new DBHome();
						db.dodajFakturu(tfBrojFakture.getText(), tfNazivFirme.getText(), tfDatum.getText(), tfDatumIsporuke.getText(), tfDatumValute.getText(),
				...
				else
				{
					...
					if (postoji)
					{
						...
						if (odabir == JOptionPane.YES_OPTION)
						{
							...
							DBHome db = new DBHome();
							db.dodajArtikal(tfRedniBroj.getText(), tfSifra.getText(), tfNazivArtikla.getText(), tfJM.getText(), tfKolicina.getText(),
		...
		JLabel lblNewLabel_1 = new JLabel("U\u010Ditaj artikle putem brojfa fakture:");
		lblNewLabel_1.setForeground(Color.WHITE);
		lblNewLabel_1.setFont(new Font("Calibri", Font.BOLD, 14));
		lblNewLabel_1.setBounds(653, 385, 233, 28);
		ulaznaFaktura.getContentPane().add(lblNewLabel_1);
		
		tfUcitajArtikle = new JTextField();
		tfUcitajArtikle.addKeyListener(new KeyAdapter() {
			@Override
			public void keyReleased(KeyEvent arg0) 
			{
				if (!tfUcitajArtikle.getText().isEmpty())
				{
					Timer timer = new Timer();
					timer.schedule(new TimerTask()
					{
						...
						public void run()
						{
							...
							//ovdje nastavi i nisi zavrsio metod u DBHome
						}
					}, 2000);
				}
			}
		});
		tfUcitajArtikle.setBounds(895, 389, 163, 20);
		ulaznaFaktura.getContentPane().add(tfUcitajArtikle);
		tfUcitajArtikle.setColumns(10);
	...
	public boolean provjeriRedniBroj(String redniBroj, String brojFakture)
	{
		...
		DBHome db = new DBHome();
		postoji = db.provjeriBrojArtikla(redniBroj, brojFakture);
