...
public class NovaIzlaznaFaktura {
	...
	public volatile boolean dovoljnoArtikala;
	...
	public static void main(String[] args) {
		try {
            ...
            java.util.logging.Logger.getLogger(NovaIzlaznaFaktura.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					NovaIzlaznaFaktura window = new NovaIzlaznaFaktura();
	...
	public NovaIzlaznaFaktura() throws ParseException {
		initialize();
	}
	...
	private void initialize() throws ParseException {
		...
		JLabel lPostojiFaktura = new JLabel("Broj une\u0161ene fakture ve\u0107 postoji u bazi. Ukucajte novi!");
		lPostojiFaktura.setFont(new Font("Calibri", Font.PLAIN, 16));
		lPostojiFaktura.setForeground(Color.WHITE);
		lPostojiFaktura.setHorizontalAlignment(SwingConstants.CENTER);
		lPostojiFaktura.setBounds(641, 201, 427, 88);
		lPostojiFaktura.setBorder(borderCrveni);
		lPostojiFaktura.setVisible(false);
		ulaznaFaktura.getContentPane().add(lPostojiFaktura);
		...
		JLabel lSlijedecaProvjera = new JLabel("Slijede\u0107a provjera za 5 sekundi!");
		lSlijedecaProvjera.setForeground(Color.WHITE);
		lSlijedecaProvjera.setBounds(772, 258, 233, 14);
		lSlijedecaProvjera.setVisible(false);
		ulaznaFaktura.getContentPane().add(lSlijedecaProvjera);
		...
		tfBrojFakture.addKeyListener(new KeyAdapter() 
		{
			@Override
			public void keyReleased(KeyEvent arg0)  //idalje radi query onoliko puta koliko karaktera ima ali ne steka unosenje slova
			{ 										//trebat ce nekad nac bolje rijesenje
				if (!tfBrojFakture.getText().isEmpty())
				{
					...
					timer.schedule(new TimerTask()
					{
						...
						public void run()
						{
							...
							rezultatPostojiBrojFakture = db.provjeriBrojIzlazneFakture(tfBrojFakture.getText());
							if (rezultatPostojiBrojFakture)
							{
								lPostojiFaktura.setVisible(true);
								lSlijedecaProvjera.setVisible(true);
							}
							else 
							{
								lPostojiFaktura.setVisible(false); 
		...
		JLabel lblDodavanjeFakture = new JLabel("Dodavanje fakture");
		...
		lSpremiFakturu.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) 
			{
				...
				else
				{
					...
					{
						DBHome db = new DBHome();
						db.dodajIzlaznuFakturu(tfBrojFakture.getText(), tfNazivFirme.getText(), tfDatum.getText(), tfDatumIsporuke.getText(), tfDatumValute.getText(),
		...
		JLabel lNemaDovoljnoArtikala = new JLabel("Nemate dovoljno artikala na stanju!");
		lNemaDovoljnoArtikala.setForeground(Color.WHITE);
		lNemaDovoljnoArtikala.setFont(new Font("Calibri", Font.PLAIN, 16));
		lNemaDovoljnoArtikala.setHorizontalAlignment(SwingConstants.CENTER);
		lNemaDovoljnoArtikala.setBounds(641, 300, 427, 88);
		ulaznaFaktura.getContentPane().add(lNemaDovoljnoArtikala);
		lNemaDovoljnoArtikala.setVisible(false);
		lNemaDovoljnoArtikala.setBorder(borderCrveni);
		...
		tfKolicina.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent arg0) 
			{
				if (!tfKolicina.getText().isEmpty() && !tfNazivArtikla.getText().isEmpty())
				{
					Timer timer = new Timer();
					timer.schedule(new TimerTask()
					{
						...
						{
							DBHome db = new DBHome();
							if (db.provjeriStanjeArtikla(tfNazivArtikla.getText(), tfKolicina.getText()))
							{
								lNemaDovoljnoArtikala.setVisible(false);
							}
							else
							{
								lNemaDovoljnoArtikala.setVisible(true);
							}
						}
					}, 2000);
				}
			...
			{	
				...
				else
				{
					...
					if (postoji)
					{
						...
						if (odabir == JOptionPane.YES_OPTION && dovoljnoArtikala)
						{
							System.out.println("Broj fakture artikla:"+tfBrojFaktureArtikal.getText());
						
							DBHome db = new DBHome();
							db.dodajIzlazniArtikal(tfRedniBroj.getText(), tfSifra.getText(), tfNazivArtikla.getText(), tfJM.getText(), tfKolicina.getText(),
